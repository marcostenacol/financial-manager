...
**Conhecidos, ainda não corrigidos** (log para não perder de vista em passagem futura):

- **`Dockerfile` da API roda `npm run dev` (`tsx watch`) mesmo nos profiles `main`/`homolog`**: single-stage, roda como `root`, instala todas as `devDependencies` em produção. Deveria ser multi-stage com build real (`tsc` → `node dist/server.js`), usuário não-root e só `dependencies` de produção. Não corrigido nesta passagem — mudança de infraestrutura maior, fora do escopo de segurança pontual desta auditoria.
...
- **UI com 84 problemas reais de ESLint** (`npm run lint` em `financial-manager-ui/`), incluindo um bug real em `WalletsPage.tsx`: chama `loadWallets()` antes de sua declaração (uso antes da definição em escopo de função/hook) — **não corrigido nesta passagem** por instrução explícita; sinalizado aqui para não se perder. Ver também Débito Técnico acima sobre ausência de lint/CI real bloqueando isso hoje.
